...
def _compute_scores(dist_matrix, y):
    ...
    for idx_shape in range(n_shapelets):
        ...
        # if dist_shape[idx_shape][0] == dist_shape[idx_shape][1]:
        #     print(f"dist_shape[idx_shape][0] == dist_shape[idx_shape][1]")
        #     print(dist_shape[idx_shape][0])
        #     print(flush=True)
        #     assert False
...
class ShapeletsMV():
    ...
    def fit_all(self, X, y, n_shapelets, window_size, window_step, n_channels, X_valid_length_list, n_jobs,
        ...
        return shapelets, scores, info_gains, start_idx, end_idx, series_idx,[info_gains,count_big,count_small]
    ...
    def _fit_one(self, i, X, y, n_shapelets, window_size, window_step, n_channels, X_valid_length_list,
        ...
        # mine shapelets from one time series in the data set

        # find all candidate shapelets in x
        x = np.array([X[ch][i] for ch in prange(n_channels)])
        # todo:希望能找出一些模糊的更有代表性的shapelets，而不是真实值
        shapelets, start_idx, end_idx = _extract_all_shapelets_mv(x, window_size, window_step, n_channels,
                                                                  X_valid_length_list[i])

        # find the distance of all candidate shapelets with all other series in X
        # dist_matrix[series_idx][shapelet_idx]
        # dist_matrix是X所有样本到x选出的所有shapelets的距离矩阵
        # print("------------------Finish extract_all_mv-------------------")

        dist_matrix = _distances_all_mv(X, window_size, shapelets, n_channels, X_valid_length_list,
                                        evolution_event_selection)

        dist_matrix = np.array(dist_matrix)
        # print("------------------Finish distances_all_mv-------------------")

        ratios = _compute_scores(dist_matrix, y)
        # 增加计算information gain的函数，并且将information gain作为额外的参数，传给keep_top_shapelet
        # 可能的参考：https://git.tex.nju.edu.cn/60fc9ee3-c8fb-4e73-8d0f-b8001d38c642.git
...
if __name__ == '__main__':
    ...
    sh = ShapeletsMV()
    # sh.fit_one(2, X, np.array([0, 1, 0]), 5, 10, 1, 2, np.array([15, 12, 100]))
    shapelets, scores, start_idx, end_idx, series_idx,list,log_list = sh.fit_all(X, np.array([0, 1, 0]), 5, 10, 1, 2,
    ...
    # # print(X)
    # shapelets, start_idx, end_idx = _extract_all_shapelets_mv(x3, 10, 1, 2, 13)
    # # print(shapelets)
    # # dist_all_shapes = _distances_single_mv(x2, 10, shapelets, 2, 10)
    # # print(dist_all_shapes)
    # dist_matrix = _distances_all_mv(X, 10, shapelets, 2, np.array([15, 12, 13]))
