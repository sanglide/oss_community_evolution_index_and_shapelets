...
def ten_fold_cross_validation(repos, x, y, model):
    ...
    for k, (train, test) in enumerate(kfold):
        ...
        y_scores=model.predict_proba(pd.DataFrame(x.iloc[test]))[:, 1]
        y_pred_sum.extend(y_pred)
        y_true_sum.extend(y.iloc[test])
        y_scores_sum.extend(y_scores)
        repos_sum.extend(repos.iloc[test])
    return y_pred_sum, y_true_sum,y_scores_sum, repos_sum
...
def execute_classify_direct(label_period_months, forecast_gap_months, data_period_months,
    ...
    for i in range(len(models)):
        y_pred_sum, y_true_sum,y_scores_sum = direct_classify(x_train, y_train, x_test, y_test, models[i])
        ...
        cs.report(test_repos, y_true_sum, y_pred_sum,y_scores_sum, label_period_months, forecast_gap_months, data_period_months,
...
def execute_classify_ten_fold(label_period_months, forecast_gap_months, data_period_months,
    ...
    for i in range(len(models)):
        y_pred_sum, y_true_sum,y_scores_sum, repos_sum = ten_fold_cross_validation(test_repos, x, y, models[i])
        cs = ClassifySimpleDist()
        cs.load(f"{result_folder_path}{shapelet_file_name}.model")
        cs.report(repos_sum, y_true_sum, y_pred_sum,y_scores_sum, label_period_months, forecast_gap_months, data_period_months,
                  result_folder_path, evolution_event_selection,
...
# def script_classification_ml(time_of_execution):
        ...
        for forecast_gap_months in list_forecast_gap_months:
            for data_period_months in list_data_period_months:
                for evolution_event_selection in list_evolution_event_selection:
                    util.test_kill_and_exit()
                    window_size = list_window_size[0]  # this is a dummy value
